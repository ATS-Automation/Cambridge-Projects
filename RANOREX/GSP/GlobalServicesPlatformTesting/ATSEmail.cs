...
*/        
        ...
        public void SendMail(string reportPath)
        {
            ...
            try 
            {
            		...
            		subject = "GSP Regression Test: " + TestSuite.Current.SelectedRunConfig.Name.ToString() + " on " + Environment.MachineName +  "_" + sResult;
            	

            	
            		addressTo = "anpatel@atsautomation.com; dmcintyre@atsautomation.com; mphelan@atsautomation.com";
            	...
            	if (System.IO.File.Exists(reportPath))
           		{
           		 	...
           		 	MailHelper.SendMessage(body, addressTo, subject, "relay2.atsna.atsauto.net", addressFrom, "GSP Test", new [] { reportPath });
           		// 	MovePDF(reportPath);
           		// 	WriteResults(sResult);
            	}
        ...
        {
        	string PDFfolderpath = @"\\ca01a9001\PGMIS\Deployment_DEV\Ranorex\JDE_Heartbeat\PDF";
        	...
        	{
        		Report.Log(ReportLevel.Info, "Failed to find or created PDF directory");
        	}
        ...
        public void WriteResults(string TestResultStatus)
        {
        	string testResultsDirectory = @"\\ca01a9001\PGMIS\Deployment_DEV\Ranorex\JDE_Heartbeat\TestResults\";
        	if (!(Directory.Exists(testResultsDirectory)))
        		{
        			Directory.CreateDirectory(testResultsDirectory);
        		}
        	string parameters = "";
        	foreach (var item in TestSuite.Current.Parameters)
        	{
        		if (!item.Value.IsEmpty() && item.Value != "")
        		{
        			parameters = parameters + item.Value + " seconds\t\t" + item.Key + "\r\n";
        		}
        	}
        	string text = GetDivision(System.Environment.MachineName) + "\t\t" + TestResultStatus + "\t\t" + System.DateTime.Now.ToString("MM/dd/yyyy h:mm tt") + "\r\n" + parameters;
