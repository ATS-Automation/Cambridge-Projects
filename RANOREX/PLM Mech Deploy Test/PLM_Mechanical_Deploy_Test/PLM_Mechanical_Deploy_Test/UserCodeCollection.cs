    ...
    public class UserCodeCollection
    {
    	...
    	public static void Click_Expand_SW_Addin(string textCaption, int timeOut)
    	{
           ...
           try
           {
               ...
               //element.Click();
               //element.Expand();
               element.UseEnsureVisible = false;
               element.MoveTo();
               Delay.Seconds(2);
               element.Click();
               Delay.Seconds(2);
              ...
              // element.DoubleClick();
               stopwatch.Stop();
               element.UseEnsureVisible = null;
               Report.Log(ReportLevel.Info, "Successfully expanded " + textCaption +" button, Time Elapsed: " + stopwatch.Elapsed);
    	...
    	/// </summary>
    	[UserCodeMethod]
    	public static void Click_Expand_SW_Addin_ProjectsBOM(string textCaption, int timeOut)
    	{
    	    Form swForm = @"/form[@title~'^SolidWorks' or @title~'^SOLIDWORKS']";
           //string xpathText = @".//text[@caption='" + textCaption +"']/../../checkbox[@automationid='Expander']";
           string xpathText = @".//text[@caption~'" + textCaption +"']/../parent::treeitem";
    	...
    	public static void Find_SW_Addin_Treeitem_Right_Click(string treeItemTextContains, int timeOut)
    	{
    		Form swForm = @"/form[@title~'^SolidWorks' or @title~'^SOLIDWORKS']";
          	...
          	//string xpathTreeitem = @".//tree[@automationid='wpf_treeView' or @automationid='wpf_tree_view']//text[@text~'" + treeItemTextContains +"']";
    	...
    	/// </summary>
    	[UserCodeMethod]
    	public static void Find_SW_Addin_Treeitem_Left_Click(string treeItemTextContains, int timeOut)
    	{
    		Form swForm = @"/form[@title~'^SolidWorks' or @title~'^SOLIDWORKS']";
          	//string xpathTreeitem = @".//tree[@automationid='wpf_treeView']//text[@text~'" + treeItemTextContains +"']";
          	//string xpathTreeitem = @".//tree[@automationid='wpf_treeView' or @automationid='wpf_tree_view']//text[@text~'" + treeItemTextContains +"']";
          	...
          	{
          		Ranorex.Text treeitem = swForm.FindSingle<Ranorex.Text>(xpathTreeitem, Duration.FromMilliseconds(timeOut));
          		treeitem.Click(WinForms.MouseButtons.Left);
          		stopwatch.Stop();
          		Report.Log(ReportLevel.Info, "Successfully found treeitem: " + treeItemTextContains + " Time Elapsed: " + stopwatch.Elapsed);
          	}
          	catch(Exception ex)
    	...
    	/// </summary>
    	[UserCodeMethod]
    	public static void FindCheckOut(string cellTextofFileName, string columnName)
    	{
    	...
    	{
    		Form swForm = @"/form[@title~'^SOLIDWORKS']";
        	string xpathStatusBarText = @".//statusbar[@accessiblename='statusStrip']//text[@text~'Success']";
        	Stopwatch stopwatch = Stopwatch.StartNew();
        	try
            {
          		Ranorex.Text statusBar = swForm.FindSingle<Ranorex.Text>(xpathStatusBarText, Duration.FromMilliseconds(180000));
          		statusBar.MoveTo();
          		stopwatch.Stop();
          		Report.Log(ReportLevel.Success, "Successfully found Staus Bar, Time Elapsed: " + stopwatch.Elapsed);
            }
            catch(Exception ex)
            {
           		stopwatch.Stop();
           		Report.Log(ReportLevel.Warn, ex.Message + " Failed to find Status Bar in Elapsed time: " + stopwatch.Elapsed);
            }
    	...
    	/// </summary>
    	[UserCodeMethod]
    	public static void Find_ComboBox_Click(string comboboxAutomationID, int timeOut)
    	{
    		Form swForm = @"/form[@title~'^SolidWorks' or @title~'^SOLIDWORKS']";
    		string xPathComboBox = @".//combobox[@automationid~'" + comboboxAutomationID + "']";
    		Stopwatch stopwatch = Stopwatch.StartNew();
    		try
           	{
               Ranorex.ComboBox element = swForm.FindSingle<Ranorex.ComboBox>(xPathComboBox, Duration.FromMilliseconds(timeOut));
               element.Click();
               stopwatch.Stop();
               Report.Log(ReportLevel.Info, "Successfully found ComboBox: " + comboboxAutomationID +" Time Elapsed: " + stopwatch.Elapsed);
           	}
           catch(Exception ex)
           	{
           		stopwatch.Stop();
           		Report.Log(ReportLevel.Failure, ex.Message + "  Failed to find " + comboboxAutomationID + " Time Elapsed: " + stopwatch.Elapsed + " ComboBox xpath: " + xPathComboBox);
           	}		
    	}
    	...
    	/// </summary>
    	[UserCodeMethod]
    	public static void RightClick_Img_SWAddin_Image_Recog(string imageName, Boolean continueOnFail)
    	{
    		Stopwatch sw = Stopwatch.StartNew();
    		Form swForm = @"/form[@title~'^SolidWorks' or @title~'^SOLIDWORKS']";
    		string path = @"\\ca01a9001\pgmis\Deployment\Ranorex\" + imageName;
    		...
    		{
    			var searchImage = Ranorex.Imaging.Load(path);
    			swForm.Click(WinForms.MouseButtons.Right, new Location(searchImage, new Imaging.FindOptions(0.97)));
    			sw.Stop();
    			Report.Log(ReportLevel.Info, "Match for Image: " + imageName + " Found, Time Elapsed: " + sw.Elapsed);
    			Delay.Seconds(1.0);
    		}
    	...
    	/// </summary>
    	[UserCodeMethod]
    	public static void WaitFor_Image_SWAddin(string imageName, double searchIntervalSeconds, double timeoutSeconds)
    	{
    		Stopwatch sw = Stopwatch.StartNew();
    		Form swForm = @"/form[@title~'^SolidWorks' or @title~'^SOLIDWORKS']";
    		string path = @"\\ca01a9001\pgmis\Deployment\Ranorex\" + imageName;
    		...
    		{
    			var searchImage = Ranorex.Imaging.Load(path);
    			if((!(Ranorex.Imaging.Contains(swForm, searchImage, new Imaging.FindOptions(0.97)))))
    			   {
		    			do
		    			{
		    				
		    				Delay.Seconds(searchIntervalSeconds);
		    				Report.Log(ReportLevel.Info, "Did not find image: " + imageName + " Retrying...");
		    				i++;
		    				
		    			
		    			} while((!(Ranorex.Imaging.Contains(swForm, searchImage, new Imaging.FindOptions(0.97))))&& i<(int)(timeoutSeconds/searchIntervalSeconds));
    			   }
    			swForm.MoveTo(new Location(searchImage, new Imaging.FindOptions(0.97)));
    			sw.Stop();
    			Report.Log(ReportLevel.Info, "Match for Image: " + imageName + " Found, Time Elapsed: " + sw.Elapsed);
    			Delay.Seconds(1.0);
    		}
    	...
    	/// </summary>
    	[UserCodeMethod]
    	public static void Checkbox_SWAddin_Check(string cbAutomationId, int timeOut)
    	{
    		
    	   Form swForm = @"/form[@title~'^SolidWorks' or @title~'^SOLIDWORKS']";
           string xpathCb = @".//checkbox[@automationid='" + cbAutomationId +"']";
           Stopwatch stopwatch = Stopwatch.StartNew();
           try
           {
               Ranorex.CheckBox element = swForm.FindSingle<Ranorex.CheckBox>(xpathCb, Duration.FromMilliseconds(timeOut));
               element.Check();
               stopwatch.Stop();
               Report.Log(ReportLevel.Info, "Successfully found " + cbAutomationId +" checkbox, Time Elapsed: " + stopwatch.Elapsed);
           }
           catch(Exception ex)
           {
           		stopwatch.Stop();
           		Report.Log(ReportLevel.Failure, ex.Message + "  Failed to find " + cbAutomationId + " Time Elapsed: " + stopwatch.Elapsed + " String xPathCb: " + xpathCb);
           }
    	...
    	/// </summary>
    	[UserCodeMethod]
    	public static void Expand_Checkbox_SWAddin_wTreeItemText(string textCaption, int timeOut)
    	{
    		 Form swForm = @"/form[@title~'^SOLIDWORKS']";
           string xpathText = @".//text[@caption~'" + textCaption +"']/../../checkbox[@automationid='Expander']";
           //string xpathText = @".//text[@caption~'" + textCaption +"']/../../../parent::treeitem";
          // string xpathText = @".//text[@caption~'" + textCaption +"']";
           ...
           {
           		stopwatch.Stop();
           		Report.Log(ReportLevel.Warn, ex.Message + "  Failed to expand " + textCaption + " button, Time Elapsed: " + stopwatch.Elapsed + " String xPathText: " + xpathText);
           }
    	}
    }
}
